...
protected_mode:
    ...
    mov byte [0xB8000 + 0 + 160], 'P'     ; Character
    mov byte [0xB8000 + 1 + 160], 0x0F    ; Attribute (white on black)
    
    ; Print "32-bit Protected Mode" message
    mov esi, protected_mode_msg
    mov edi, 0xB8000 + 160      ; Second line (80*2 bytes per line)
    call print_string_pm
    
    std
    ; fix this part so it jumps back to the bootloader in 16bit real mode 
    jmp 0x7C00
...
display_protected_mode_status:
    ...
    mov byte [0xB8000 + 0 ], 'M' ; "M"
    mov byte [0xB8000 + 1 ], 0x0A ; Green on black
    mov byte [0xB8000 + 2 ], 'o' ; "o"
    mov byte [0xB8000 + 3 ], 0x0A ; Green on black
    mov byte [0xB8000 + 4 ], 'd' ; "d"
    mov byte [0xB8000 + 5 ], 0x0A ; Green on black
    mov byte [0xB8000 + 6 ], 'e' ; "e"
    mov byte [0xB8000 + 7 ], 0x0A ; Green on black
    mov byte [0xB8000 + 8 ], ':' ; ":"
    mov byte [0xB8000 + 9 ], 0x0A ; Green on black
    mov byte [0xB8000 + 10], al ; "1" (PE bit value)
    mov byte [0xB8000 + 11], 0x0A ; Green on black
    
    ret

; Function: clear_screen
; Purpose: Clears the entire screen by filling video memory with zeros
; Input: None
; Output: None
...
clear_screen:
    ...
    cld                     ; Clear direction flag (ensure we increment)
    mov edi, 0xB8000        ; Point to start of video memory
    xor eax, eax            ; Set EAX to 0 (black on black)
    mov ecx, 1000           ; 80*25 characters / 2 characters per DWORD = 1000
    rep stosd               ; Repeat ECX times: store EAX at [EDI] and add 4 to EDI
